const { pool, pedidoModel } = require('../models/pedidoModel');

const pedidoController = {
    criarPedido: async (req, res) => {
        try {
            const { id_cliente, valor_pedido, data_pedido, id_produto, quantidade, valor_item } = req.body;

            if (!id_cliente | !valor_pedido | !data_pedido | !id_produto | !quantidade | !valor_item) {
                return res.status(400).json({
                    message: 'Verifique os dados enviados e tente novamente'
                });
            }

            const resultado = await pedidoModel.insertPedidos(id_cliente, valor_pedido, data_pedido, id_produto, quantidade, valor_item);
    ...
    excluirItem: async (req, res) => {
        try {
            ...
            if (!idPedido || !Number.isInteger(idPedido) || !idItem || !Number.isInteger(idItem)) {
                return res.status(400).json({
                    message: 'Forneça um identificador válido'
                });
            }
            ...
            if (itemSelecionado.length === 0) {
                return res.status(200).json({
                    message: 'Item não localizado na base de dados'
                });
            }
